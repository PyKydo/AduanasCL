  ...
  forms.forEach((form) => {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (
        ...
        window.validationSystem.validateAllFields(this)
      ) {
        ...
        const firstError = this.querySelector(
          '.error, .validation-message[style*="block"]'
        );
        if (firstError) {
...
// Funciones para manejar el guardado y la obtención de trámites desde localStorage
const TRAMITES_STORAGE_KEY = "tramitesSubmissions";

function getTramiteSubmissions() {
  try {
...
function saveTramiteSubmission(formData) {
  try {
    const submissions = getTramiteSubmissions();

    // Evitar duplicados exactos basados en todos los datos del formulario
    const isDuplicate = submissions.some(
      (sub) =>
        sub.tipoTramite === formData.tipoTramite &&
        JSON.stringify(sub.datos) === JSON.stringify(formData.datos)
    );

    if (isDuplicate) {
      console.warn(
        "Se intentó guardar un trámite duplicado. Operación cancelada."
      );
...
function handleFormSubmission(form) {
  ...
  if (submitButton) {
    ...
    // Fallback para rehabilitar el botón
    setTimeout(() => {
  ...
  // Simular un pequeño retraso y luego procesar
  setTimeout(() => {
    // Generar número de trámite estándar
    const timestamp = Date.now();
    const tramiteData = {
      numeroTramite: `TR-${timestamp}`,
      tipoTramite: form.id || "general",
      estado: "Enviado",
    ...
    // Guardar el trámite
    const saveResult = saveTramiteSubmission(tramiteData);

    if (saveResult.duplicate) {
      showSuccessMessage(
        form,
        "Este trámite ya ha sido registrado anteriormente."
      );
    } else if (saveResult.success) {
      showSuccessMessage(
        form,
        `Trámite enviado con éxito. Número: ${tramiteData.numeroTramite}`
      );
    } else {
      // Aquí podrías tener un `showErrorMessage` si lo deseas
    }

    // Limpiar y rehabilitar el formulario
    form.reset();
    if (submitButton) {
      submitButton.disabled = false;
      // Restaurar texto original del botón
      const buttonText = form.id === "seguimiento-form" ? "Buscar" : "Enviar";
      submitButton.innerHTML = `<span class="material-symbols-outlined">send</span> ${buttonText}`;
    }

    // Limpiar los indicadores de validación
